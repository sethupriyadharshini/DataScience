...
class Univariate():
    ...
    def dataFrameTable(dataset,quan):
        descriptive = pd.DataFrame(index=['Mean','Median','Mode','Min','Q1-25%','Q2-50%','Q3-75%','99th','Q4-100%','IQR','1.5rule','Lesser','min','max','Greater','Skew','Kurtosis','Var','SD'],columns=quan) #it creates table with given index value in row and column value
    ...
    def simpleImputerMethod(imputerType,strategy,dataset):
        imp = SimpleImputer(missing_values=np.nan, strategy=strategy)
        imp.fit(dataset[imputerType])
        da_quan=imp.transform(dataset[imputerType])
        quan_dataset=pd.DataFrame(da_quan,columns=imputerType)
        return quan_dataset
